...
function formatTimeAgo(
  ...
  if (seconds < 60) return t("projectSwitcher.time.justNow");
  if (seconds < 3600) {
    const mins = Math.floor(seconds / 60);
    return t("projectSwitcher.time.minutesAgo", { count: mins });
  }
  if (seconds < 86400) {
    const hours = Math.floor(seconds / 3600);
    return t("projectSwitcher.time.hoursAgo", { count: hours });
  }
  const days = Math.floor(seconds / 86400);
  return t("projectSwitcher.time.daysAgo", { count: days });
}
